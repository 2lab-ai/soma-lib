...
function toWallClock(date: Date, mode: 'local' | 'utc'): Date {
  if (mode === 'utc') return date;
  return new Date(date.getTime() - date.getTimezoneOffset() * 60_000);
}

/** Calendar-minute key of a Date under the given wall-clock mode. */
function minuteKey(date: Date, mode: 'local' | 'utc'): string {
...
export function createMinuteCronJobFactory(options: MinuteCronJobOptions = {}): CronJobFactory {
  ...
  return (cronExpression: string, onTick: () => void | Promise<void>): CronJobHandle => {
    ...
    const check = () => {
      ...
      if (!matchesCronExpression(cronExpression, toWallClock(current, wallClock))) return;
      lastFiredMinute = key;
      try {
        const result = onTick();
    ...
    return {
      ...
      nextRun() {
        ...
        const firstBoundary = Math.floor(start.getTime() / 60_000) * 60_000 + 60_000;
        for (let i = 0; i < scanLimit; i++) {
          const candidate = new Date(firstBoundary + i * 60_000);
          if (matchesCronExpression(cronExpression, toWallClock(candidate, wallClock))) {
            return candidate;
          }
        }
        return null;
