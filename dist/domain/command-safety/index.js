...
exports.DANGEROUS_RULES = exports.EXECUTION_DISPATCH_RULES = void 0;
exports.createRuleSet = createRuleSet;
exports.matchRules = matchRules;
exports.rulesByIds = rulesByIds;
exports.overridableMatchedRuleIds = overridableMatchedRuleIds;
...
function createRuleSet(rules) {
    ...
    return {
        ...
        overridableRulesByIds(ruleIds) {
            return ruleIds
                .map((id) => byId.get(id))
                .filter((r) => r !== undefined && r.sessionOverridable);
        },
    };
}
...
const EXECUTION_DISPATCH_PATTERNS = [
    {
    ...
    },
    {
        ...
        regex: new RegExp(`\\|\\s*(?:\\/(?:usr\\/)?(?:local\\/)?(?:s?bin)\\/(?:env\\s+(?:-\\S+\\s+)*)?|env\\s+(?:-\\S+\\s+)*)(?:${ALL_INTERPRETERS})\\b`, 'i'),
    },
    {
        id: 'pipe-to-busybox',
    ...
    },
    {
        ...
        regex: new RegExp(`\\|\\s*(?:\\/(?:usr\\/)?(?:local\\/)?(?:s?bin)\\/)?env\\s+(?:-\\S+\\s+)*(?:${ALL_INTERPRETERS})\\b`, 'i'),
    },
];
/**
 ...
 */
exports.EXECUTION_DISPATCH_RULES = EXECUTION_DISPATCH_PATTERNS.map((p) => ({
    id: p.id,
    label: p.label,
    description: p.description,
    sessionOverridable: true,
    match: (cmd) => p.regex.test(cmd),
}));
/**
...
exports.DANGEROUS_RULES = [
    ...
    ...exports.EXECUTION_DISPATCH_RULES,
...
const CANONICAL = createRuleSet(exports.DANGEROUS_RULES);
/**
 ...
 */
function matchRules(command, ctx = {}) {
 ...
 */
function overridableMatchedRuleIds(command) {
 ...
 */
function overridableRulesByIds(ruleIds) {
 ...
 */
function isCrossUserAccess(command, currentUserId) {
 ...
 */
const SSH_PATTERNS = [/\bssh\b/, /\bscp\b/, /\bsftp\b/, /\brsync\b.*\b-e\s+['"]?ssh/];
/**
